...
const VerticalStep = React.forwardRef((props, ref) => {
  ...
  return (
    ...
    >
      ...
      >
        {isStepperDot ? (
          <Dot height={50} width={50} className="ml-[-6px] z-10" />
        ) : (
          <StepButtonContainer {...{ isLoading: localIsLoading, isError: localIsError, ...props }}>
            <StepIcon
              ...
              checkIcon={checkIcon}
              errorIcon={errorIcon}
              isStepperDot={isStepperDot}
            />
          </StepButtonContainer>
        )}
        <StepLabel label={label} description={description} reason={reason} {...{ isCurrentStep, opacity }} />
      </div>
      <div
        className={cn(
